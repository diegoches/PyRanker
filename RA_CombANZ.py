...
#   The CombANZ algorithm divide the sum of the objects similarities in each ranked list by the number of non zero similarities of each object, and then it order them in descending order.
#   \cite Fox:1994 .
class RA_CombANZ(RankAggregation):

	## \fn rank 
	#  The rank function uses the CombANZ algorithm to compute a new ranked list for the query object.
	...
	def rank(self,*tp_param):
		...
		# Divides the new similarity of each element by its quantity of non zero similarities.
