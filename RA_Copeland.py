...
#   The Copeland algorithm compare each pair of objects and determine which one is more preferred. The more preferred object is awarded with 1 point. If there is a tie, each object is awarded with 0.5 point. After all pairwise comparisons are made, the candidate with the most points (the most pairwise wins) is declared the winner.
#   \cite Lippman:2012 .
class RA_Copeland(RankAggregation):

    ## \fn rank 
    #  The rank function uses the Copeland algorithm to compute a new ranked list for the query object.
    ...
    def rank(self,*tp_param):
        ...
        #Initialize the score matrix (comparisons) and compute the scores for the first rank aggregation
        ...
        ls_result = sorted(ls_result, key=lambda ls_result: ls_result['sim'], reverse=True)
    ...
    ## \fn __win
    #  The win function returns which object is more similar (better position) in the ranked list.
    ...
    #  \return A floating point number bigger than 0 if the first item is more similar; smaller than 0 if the second is more similar; 0 if both have the same similarity.
