...
#   The CombMED algorithm compute the mean of the similarities of each object in each ranked list, and then it order them in descending order.
#   \cite Fox:1994 .
class RA_CombMED(RankAggregation):

	## \fn rank 
	#  The rank function uses the CombMED algorithm to compute a new ranked list for the query object.
	...
	def rank(self,*tp_param):
		...
		# Sums the similarities of each object in a dictionary where each key is the id of each object.
