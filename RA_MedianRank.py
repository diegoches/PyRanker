...
#   The Median algorithm finds the median of the positions from each object in each ranked list, and then it order them in ascending order.
#   \cite Fagin:2003 .
class RA_MedianRank(RankAggregation):

	## \fn rank 
	#  The rank function uses a Median algorithm to compute a new ranked list for the query object.
	...
	def rank(self,*tp_param):
		...
		for i in range(len(dc_map)):
			...
			ls_finalRank.append({'id':dc_map[i][0],'rank':i+1,'sim':norm_median})
