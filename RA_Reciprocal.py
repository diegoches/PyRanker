...
##  The RA_Reciprocal class handles the Reciprocal Rank Fusion algorithm for Rank Aggregation. 
#   This class inherits from the RankAggregation class, and implements the rank() function. 
#   The Reciprocal Rank Fusion (RRF) algorithm is a simple method for combining the document rankings from multiple IR systems. It uses the rank information for computing a similarity score between the query object and another one.
#   \cite Cormack:2009 . 
class RA_Reciprocal(RankAggregation):
	
	## \fn rank 
	#  The rank function uses the RRF algorithm to compute a new ranked list for the query object.
	#  \param tp_param Is a constant that mitigates the impact of high rankings by outlier systems.
